...
define('FILE_NAME', './wx');
define('LOG_FILE', FILE_NAME . '.log');
define('CACHE_FILE', FILE_NAME . '.cache');
...
class WeixinChat {
	...
	public function getRev () {
		...
		if ( $postStr ) {
			...
			file_put_contents(LOG_FILE, $postStr . PHP_EOL . PHP_EOL . var_export($this->_receive, true) . PHP_EOL . PHP_EOL, FILE_APPEND);
		}
		return $this;
	}
	...
	public function getRevText () {
		if ( isset($this->_receive['Content']) )
			return trim($this->_receive['Content']);
		elseif ( isset($this->_receive['Recognition']) )
			return trim($this->_receive['Recognition']);
		else
			return false;
	}
	...
	public function text ( $content = '' ) {
		...
		$this->_reply = sprintf($textTpl, $this->getRevFrom(), $this->getRevTo(), TIMESTAMP, 'text', $content);
		return $this;
	}
	...
	public function music ( $title, $desc, $musicurl, $hgmusicurl = '' ) {
		...
		$this->_reply = sprintf($textTpl, $this->getRevFrom(), $this->getRevTo(), TIMESTAMP, 'music', $title, $desc, $musicurl, $hgmusicurl);
		return $this;
	}
	...
	public function news ( $data ) {
		...
		if ( $count > 0 ) {
			foreach ( $data as $v ) {
				...
				$subText .= sprintf($tmpText, $v['title'], isset($v['description']) ? $v['description'] : '', isset($v['picUrl']) ? $v['picUrl'] : '', 
						isset($v['url']) ? $v['url'] : '');
			}
		}
		...
		$this->_reply = sprintf($textTpl, $this->getRevFrom(), $this->getRevTo(), TIMESTAMP, $count, $subText);
		return $this;
	}
	...
	public function createGroup ( $name ) {
		...
		$result = curlRequest(self::API_URL_PREFIX . self::GROUPS_CREATE_URL . 'access_token=' . $this->access_token, $this->jsonEncode($data), 
				'post');
		if ( $result ) {
			$jsonArr = json_decode($result, true);
			if (  ! $jsonArr || ( isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0 ) )
	...
	public function updateGroup ( $id, $name ) {
		...
		$result = curlRequest(self::API_URL_PREFIX . self::GROUPS_UPDATE_URL . 'access_token=' . $this->access_token, $this->jsonEncode($data), 
				'post');
		if ( $result ) {
			$jsonArr = json_decode($result, true);
			if (  ! $jsonArr || ( isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0 ) )
	...
	public function updateGroupMembers ( $openid, $to_groupid ) {
		...
		$result = curlRequest(self::API_URL_PREFIX . self::GROUPS_MEMBERS_UPDATE_URL . 'access_token=' . $this->access_token, 
				$this->jsonEncode($data), 'post');
		if ( $result ) {
			$jsonArr = json_decode($result, true);
			if (  ! $jsonArr || ( isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0 ) )
				$this->error($jsonArr);
	...
	public function sendCustomMessage ( $touser, $data, $msgType = 'text' ) {
		...
		$result = curlRequest(self::API_URL_PREFIX . self::MESSAGE_CUSTOM_SEND_URL . 'access_token=' . $this->access_token, $this->jsonEncode($arr), 
				'post');
		if ( $result ) {
			$jsonArr = json_decode($result, true);
			if (  ! $jsonArr || ( isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0 ) )
	...
	public function error ( $data ) {
		...
		file_put_contents(LOG_FILE, 'WEIXIN API errcode:[' . $this->errCode . '] errmsg:[' . $this->errMsg . ']' . PHP_EOL . PHP_EOL, FILE_APPEND);
	}
